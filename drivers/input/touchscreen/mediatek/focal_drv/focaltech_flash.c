...
static unsigned char CTPM_FW[] =
{
	...
	#include "include/firmware/SKI500_B18_FT34x7_V01_D0B_20160815_app.i"
};
...
u8 fts_check_fw_status(struct i2c_client *client)
{
    ...
    FTS_DEBUG("[UPGRADE]: << %s >> : uc_tp_vendor_id = %d, uc_chip_id = %d \n", __func__, uc_tp_vendor_id, uc_chip_id);

    if ((uc_chip_id == fts_updateinfo_curr.CHIP_ID) &&
        ((uc_tp_vendor_id == FTS_VENDOR_1_ID)|| (uc_tp_vendor_id == FTS_VENDOR_2_ID)))//call fts_flash_get_upgrade_info in probe function firstly.
    {
        fw_status = FTS_RUN_IN_APP;
        FTS_DEBUG("[UPGRADE]: << %s >> : APP OK!! \n", __func__);
    }
    else if ((uc_tp_vendor_id == FTS_VENDOR_1_ID)|| (uc_tp_vendor_id == FTS_VENDOR_2_ID))
    {
...
bool fts_check_need_upgrade(struct i2c_client *client)
{
    ...
    if (fw_status == FTS_RUN_IN_APP) //call fts_flash_get_upgrade_info in probe function firstly.
    {
        ...
        FTS_DEBUG("[UPGRADE]: << %s >> : uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n", __func__, uc_tp_fm_ver, uc_host_fm_ver);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
    ...
    FTS_DEBUG("[UPGRADE]: << %s >> : bUpgradeFlag = 0x%x\n", __func__, bUpgradeFlag);
	...
	int *fwver_in_chip, int *module_id_in_chip)
{
	...
	while (i < 2) {
		...
		*module_id_in_chip = uc_reg_value;
		...
		if(0x00 == *module_id_in_chip || 0xff == *module_id_in_chip || FTS_REG_VENDOR_ID == *module_id_in_chip) {
	...
	if(0x00 == *module_id_in_chip || 0xff == *module_id_in_chip || FTS_REG_VENDOR_ID == *module_id_in_chip) {
		...
		*module_id_in_chip = 0x57;
		*fwver_in_chip = 0x00;
	}
#endif
	if(NULL != strstr(tpd_fw_cdev.lcm_info, "DiJing")){
		printk("tpd oncell structure, lcm dijing, tp dijing.\n");
		*module_id_in_chip = 0x67;
