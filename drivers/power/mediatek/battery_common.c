...
struct wake_lock zte_inout_charger_lock;
...
static PMU_STATUS mt_battery_CheckBatteryTemp(void)
{
	...
	if (batt_cust_data.mtk_temperature_recharge_support) {
		...
		{
			battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery Under or Over Temperature !!\n\r");
			status = PMU_STATUS_FAIL;
		}
		else if (BMT_status.temperature > MIN_CHARGE_TEMPERATURE
			&& BMT_status.temperature < MAX_CHARGE_TEMPERATURE)
		{
			battery_log(BAT_LOG_CRTI, "[BATTERY] Battery is in safe Temperature !!\n\r");
...
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			if(BMT_status.charger_exist == KAL_FALSE) {
				//doumm
				wake_lock_timeout(&zte_inout_charger_lock, 10*HZ);
			}
...
static int battery_probe(struct platform_device *dev)
{
	...
	wake_lock_init(&zte_inout_charger_lock, WAKE_LOCK_SUSPEND, "zte_inout_charger_lock");
#if defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT)
