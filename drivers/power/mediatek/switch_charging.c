...
int mtk_get_dynamic_cv(unsigned int *cv)
{
	...
	if (!g_enable_dynamic_cv) {
		if (batt_cust_data.high_battery_voltage_support)
			_cv = BATTERY_VOLT_04_350000_V / 1000;
		else
...
static void set_jeita_charging_current(void)
{
	...
	//Charging Current according to Battery DataSheet
	if (g_temp_status == TEMP_NEG_10_TO_POS_0) {
		kal_bool charging_enable = KAL_FALSE;
		battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
		battery_log(BAT_LOG_CRTI, "[BATTERY] TEMP_NEG_10_TO_POS_0, Disable Charging %d\r\n",
			    g_temp_CC_value);
	}
	if (g_temp_status == TEMP_POS_0_TO_POS_10) {

		if ((BMT_status.charger_type == STANDARD_CHARGER)||(BMT_status.charger_type == NONSTANDARD_CHARGER)||(BMT_status.charger_type == APPLE_2_1A_CHARGER)) 
		{
			g_temp_CC_value = CHARGE_CURRENT_650_00_MA;	// Rsense 56, so 786mA
		...
		if ((BMT_status.charger_type == STANDARD_CHARGER)||(BMT_status.charger_type == NONSTANDARD_CHARGER)||(BMT_status.charger_type == APPLE_2_1A_CHARGER)) 
		{
			g_temp_CC_value = CHARGE_CURRENT_800_00_MA;	// Rsense 56, so 968mA
...
*/
	battery_log(BAT_LOG_CRTI,
			"select_charging_current_bcct() g_bcct_value =%d\n", g_bcct_value);
...
#endif
		battery_log(BAT_LOG_CRTI,  
			"select_charging_current_bcct() g_temp_input_CC_value = %d, g_temp_CC_value = %d\n", 
					g_temp_input_CC_value, g_temp_CC_value);
	mtk_check_aicr_upper_bound();
}
