...
#include "s5k4h9mipiraw_Sensor.h"
#include "s5k4h9mipi_otp.h"

#define PFX "S5K4H9_camera_sensor"
//#define LOG_1 LOG_INF("S5K4H9,MIPI 4LANE\n")
...
extern bool otp_wb_update_4h9(BYTE zone);
extern bool otp_lenc_update_4h9(USHORT zone);

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K4H9_SENSOR_ID,
...
{	
	  ...
	  RealSensorId=read_cmos_sensor(0x6F12);	
	  if(RealSensorId == S5K4H8_SENSOR_ID)		
	  	{          
	  	LOG_INF("Read S5K4H9 Sensor ID succeed!!!!");		  
	...
	else if(chip_id == 0xc001)
	{
		...
		LOG_INF("QYC\n");
		  write_cmos_sensor(0x6028,	0X2000);
			...
			write_cmos_sensor(0x0B00,   0x0180);//0x0180
			write_cmos_sensor(0x6F12,	0X00F0);
...
static kal_uint32 open(void)
{
	...
	sensor_init();
	otp_lenc_update_4h9(1);
	if(!otp_wb_update_4h9(1))
	{
	  otp_wb_update_4h9(2);
	}
	spin_lock(&imgsensor_drv_lock);
