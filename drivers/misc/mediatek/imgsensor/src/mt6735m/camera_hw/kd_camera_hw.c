...
#define PK_XLOG_INFO(fmt, args...)  pr_err(PFX  fmt, ##args)
...
#define VOL_1000 1000000
	...
	if (On) {
		...
		if ((currSensorName && (0 == strcmp(currSensorName,SENSOR_DRVNAME_HI843B_MIPI_RAW))))
			{
				...
				if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
					mtkcam_gpio_set(pinSetIdx, CAMRST,pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
				}
	        ...
	        else if ((currSensorName && (0 == strcmp(currSensorName,SENSOR_DRVNAME_SP2509_MIPI_RAW))))
			{
				printk(" SENSOR_DRVNAME_SP2509_MIPI_RAW--cyc-1129- \n");
				//First Power Pin low and Reset Pin Low
				//dvdd--io--avdd--mlck-pwdn-reset-
				//io and dvdd is same
				if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
		...
		{
			PK_DBG("[PowerOFF] get in---  SENSOR_DRVNAME_SP2509_MIPI_RAW \n");
			//Set Power Pin low and Reset Pin Low
			//reset--mclk--pwdn--avdd--dvdd--io--pwdn   
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
			...
			PK_DBG("[CAMERA SENSOR]  success to disenable sp2509 power \n");
			}
	}
