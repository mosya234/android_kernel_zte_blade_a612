...
static struct hw_info SubCam =
{
.hst = SUB_CAM,
.name = "HI259N-2.0Mp-FF",
.vendor = "SunRise",
.id=0xE1
};
#endif
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = HI259N_SENSOR_ID, //HI259MIPI_SENSOR_ID,  /*sensor_id = 0x2680*/ //record sensor id defined in Kd_imgsensor.h
	
	.checksum_value = 0x1aac728, //checksum value for Camera Auto Test
	
	.pre = {
		.pclk = 72000000,				//record different mode's pclk
		.linelength = 1800,				//record different mode's linelength
		.framelength = 1300,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width = 1600, //1632,		//record different mode's width of grabwindow
		.grabwindow_height = 1200, //1228,		//record different mode's height of grabwindow
	...
	.cap = {/*normal capture*/
		...
		.startx = 0,
		.starty = 0,
		.grabwindow_width = 1600, //1632,
		.grabwindow_height = 1200, //1228,
		.mipi_data_lp2hs_settle_dc = 85,//unit , ns
		.max_framerate = 300,
	},
 ...
 //Gionee: malp <2016-05-24> modify for CR01703897 bigin
	...
	.i2c_addr_table = {0x70, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};
...
static imgsensor_struct imgsensor = {
	...
	.i2c_write_id = 0x70,//record current sensor's i2c write id
};
...
{
   #if 0
   	LOG_INF("read_cmos_sensor(0x01) = %d", read_cmos_sensor(0x01));
   	LOG_INF("read_cmos_sensor(0x02) = %d", read_cmos_sensor(0x02));
	LOG_INF("read_cmos_sensor(0x03) = %d", read_cmos_sensor(0x03));
	LOG_INF("read_cmos_sensor(0x04) = %d", read_cmos_sensor(0x04));
	LOG_INF("read_cmos_sensor(0x05) = %d", read_cmos_sensor(0x05));	
	return 0xe1;
   #else
...
// MIPI_4x = 360 , ISP_clk = 144Mhz , OPCLK = 72Mhz, PXL_clk = 72 Mhz
 ...
 write_cmos_sensor(0x08, 0x54); //5a);  // pll main div : 4x ~ 127x           
...
write_cmos_sensor(0x23, 0x0e);
write_cmos_sensor(0x24, 0x00);
write_cmos_sensor(0x25, 0x10);
write_cmos_sensor(0x26, 0x00);
write_cmos_sensor(0x27, 0x0e);
write_cmos_sensor(0x28, 0x04);
write_cmos_sensor(0x29, 0xb0);		//win_height 1200
write_cmos_sensor(0x2a, 0x06);
write_cmos_sensor(0x2b, 0x40);		//win_width 1600
 ...
 write_cmos_sensor(0x4f, 0x14);	//frame_length_line 1300
...
//////////////////
 ...
 write_cmos_sensor(0x11, 0x00);	//2016.02.03    0x00 --> 0x08 en_color_blc_ofs
 ...
 write_cmos_sensor(0x17, 0x02);	//check
...
//////////////////
 ...
 write_cmos_sensor(0x12, 0x70); //2015.02.03
...
//////////////////
 ...
 write_cmos_sensor(0x11, 0x04); 	//check
 write_cmos_sensor(0x12, 0x00);
 write_cmos_sensor(0x13, 0x00);
 ...
 write_cmos_sensor(0x95, 0x00); //2014.10.23.a ramp_vrst_ctl
...
/////////////////////////
 ...
 write_cmos_sensor(0x65, 0x00);	//ramp_gain_max
...
//////////////////
 ...
 //add 20150424
 write_cmos_sensor(0x39, 0x54); //0x52);  //0x055 // lvds_bias_ctl   
 ...
 write_cmos_sensor(0x14, 0x00);  // ser_out_ctl_1      
 write_cmos_sensor(0x16, 0x12);  //check // lvds_inout_ctl1 
...
// DATA Lane Timing Control //
 ...
 write_cmos_sensor(0x1d, 0x0a);  //09);  // tlpx_time_l_dn  
 write_cmos_sensor(0x1e, 0x10);  //0f);  // hs_zero_time    
 write_cmos_sensor(0x1f, 0x0b);  // hs_trail_time   
 write_cmos_sensor(0x30, 0x07);  // l_pkt_wc_h   1600 * 5 / 4 = 2000  (MIPI Data Byte)
 write_cmos_sensor(0x31, 0xD0); //f8);  // l_pkt_wc_l   1600 * 5 / 4 = 2000  (MIPI Data Byte)
...
// MIPI_4x = 360 , ISP_clk = 144Mhz , OPCLK = 72Mhz, PXL_clk = 72 Mhz
 write_cmos_sensor(0x03, 0x00); 
 write_cmos_sensor(0x07, 0x05);  // pll enable & pre div setting	   
 write_cmos_sensor(0x08, 0x54); //0x5a);  // pll main div : 4x ~ 127x		   
 write_cmos_sensor(0x09, 0x13);  // isp div : 2/5 mipi 4x div : 1/1 mipi 1x div : 1/5  
 write_cmos_sensor(0x07, 0x85);  // pll enable & pre div setting	   
 write_cmos_sensor(0x07, 0x85);  // pll enable & pre div setting	   
 write_cmos_sensor(0x07, 0x85);  // pll enable & pre div setting	   
...
//////////////////
 write_cmos_sensor(0x03, 0x05);  // mode_page       
 write_cmos_sensor(0x39, 0x54);  //52);  // lvds_bias_ctl   
 write_cmos_sensor(0x4c, 0x20);  // hs_wakeup_size_h
 write_cmos_sensor(0x4d, 0x00);  // hs_wakeup_size_l
 write_cmos_sensor(0x4e, 0x40);  // mipi_int_time_h 
 write_cmos_sensor(0x4f, 0x00);  // mipi_int_time_l 
