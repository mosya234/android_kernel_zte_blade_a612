...
#ifdef BUILD_LK	
	dprintf(0,"%s, F10 boe td4100 boe , pin_lcd_id0= %d  pin_lcd_id1 is 0x%x \n", __func__, pin_lcd_id0,pin_lcd_id1);
#else
	printk("%s, F10 boe td4100 boe , pin_lcd_id0= %d  pin_lcd_id1 is 0x%x \n", __func__, pin_lcd_id0,pin_lcd_id1);
#endif
	if ( 1== pin_lcd_id0 && 0== pin_lcd_id1)
	{
		return 1;
	}
	else 
...
{
#ifdef BUILD_LK
	dprintf(0,"%s,lk lcm_init:boe td4100\n", __func__);
#else
	printk("lcm_init:boe td4100 \n");
#endif
...
{	
    ...
    SET_RESET_PIN(0);
    #ifdef BUILD_LK
	dprintf(0,"%s,lk :boe td4100 10ms\n", __func__);
    #else
    	printk("lcm_suspend:boe td4100 10ms \n");
    #endif	
    MDELAY(10);
...
#ifdef BUILD_LK
	dprintf(0,"%s,zhangjian lk td4100_dsi_vdo_boe_hd720: level = %d\n", __func__, level);
#else
	printk("%s,  td4100_dsi_vdo_boe_hd720 backlight: new level = 0x%x evel_high, level_low 0x%x,0x%x\n", __func__, level,level_high, level_low);
#endif       
        push_table(lcm_backlight_level_setting, sizeof(lcm_backlight_level_setting) / sizeof(struct LCM_setting_table), 1);

}

LCM_DRIVER td4100_dsi_vdo_boe_hd720_lcm_drv = 
{
	.name			= "td4100_dsi_vdo_boe_hd720",
