...
#ifdef BUILD_LK	
	dprintf(0,"%s, F10 holitech ili9881c boe , pin_lcd_id0= %d  pin_lcd_id1 is 0x%x \n", __func__, pin_lcd_id0,pin_lcd_id1);
#else
	printk("%s, F10 holitech ili9881c boe , pin_lcd_id0= %d  pin_lcd_id1 is 0x%x \n", __func__, pin_lcd_id0,pin_lcd_id1);
#endif
...
#ifdef BUILD_LK	
		dprintf(0,"%s, F10 holitech ili9881c , buffer= %d\n", __func__, buffer);
#else
		printk("%s, F10 holitech ili9881c , buffer= %d \n", __func__, buffer);
#endif
...
{
#ifdef BUILD_LK
	dprintf(0,"%s,lk lcm_init:holitech ili9881c\n", __func__);
#else
	printk("lcm_init:holitech ili9881c \n");
#endif
...
{	
    ...
    SET_RESET_PIN(0);
    #ifdef BUILD_LK
	dprintf(0,"%s,lk :holitech ili9881c 10ms\n", __func__);
    #else
    	printk("lcm_suspend:holitech ili9881c 10ms \n");
    #endif	
    MDELAY(10);
...
#ifdef BUILD_LK
	dprintf(0,"%s,zhangjian lk ili9881c_dsi_vdo_holitech_hd720: level = %d\n", __func__, level);
#else
	printk("%s,  ili9881c_dsi_vdo_holitech_hd720 backlight: new level = 0x%x evel_high, level_low 0x%x,0x%x\n", __func__, level,level_high, level_low);
#endif       
        push_table(lcm_backlight_level_setting, sizeof(lcm_backlight_level_setting) / sizeof(struct LCM_setting_table), 1);

}

LCM_DRIVER ili9881c_dsi_vdo_holitech_hd720_lcm_drv = 
{
	.name			= "ili9881c_dsi_vdo_holitech_hd720",
