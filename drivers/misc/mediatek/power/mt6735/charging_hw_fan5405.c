...
static unsigned int charging_hw_init(void *data)
{
	...
	fan5405_reg_config_interface(0x06, 0x78);

	fan5405_reg_config_interface(0x00, 0xC0);	/* kick chip watch dog */
	fan5405_reg_config_interface(0x01, 0x88);	/* TE=1, CE=0, HZ_MODE=0, OPA_MODE=0 */
	fan5405_reg_config_interface(0x05, 0x04);
	if (!charging_init_flag) {
		fan5405_reg_config_interface(0x04, 0x12);	/*177mA(56mohm)*/
...
{
	unsigned int status = STATUS_OK;
	unsigned int original_voltage = *(unsigned int *) (data);
	u16 register_value;
// there is no 4350mV for VBAT_CV_VTH fan5405,in case no register value match
    if (BATTERY_VOLT_04_350000_V == original_voltage) {
        original_voltage = BATTERY_VOLT_04_360000_V;
		battery_log(BAT_LOG_CRTI, "[gy ADD] %s, fan5405 has no 4350mV, so set %dmV\n", __func__, original_voltage);
    }
	register_value = charging_parameter_to_value(VBAT_CV_VTH, GETARRAYNUM(VBAT_CV_VTH), original_voltage);
	battery_log(BAT_LOG_CRTI, "[gy ADD] %s,cv_voltage = %d, register_value = %d\n", __func__, original_voltage, register_value);
...
static unsigned int charging_get_current(void *data)
{
	...
	fan5405_read_interface(0x4, &reg_value, 0x7, 0x4);
	battery_log(BAT_LOG_CRTI, " charging_get_current:reg_value=%d\n",reg_value);
