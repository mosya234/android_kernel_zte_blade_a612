...
#define KXTJ2_1009_CALI_LEN            (10)
#define KXTJ2_1009_MAXZ 1024
/*----------------------------------------------------------------------------*/
...
static int is_acc_calibrate_data_valid(int cali_value[KXTJ2_1009_AXES_NUM])
{
	// cali_value's unit is mg.
	int offset = (KXTJ2_1009_MAXZ * 2) / 9.8;
	if ((abs(cali_value[KXTJ2_1009_AXIS_X]) > offset) 
		|| (abs(cali_value[KXTJ2_1009_AXIS_Y]) > offset)
		|| (abs(cali_value[KXTJ2_1009_AXIS_Z]) > offset))
	{
		return 0;
	}
	return 1;
...
{
	int cnt;	
	s16 rawacc[KXTJ2_1009_AXES_NUM] = {0};
	s16 raw_sum[KXTJ2_1009_AXES_NUM] = {0};
	int cali_data[KXTJ2_1009_AXES_NUM] = {0};
	int i = 0;
	struct kxtj2_1009_i2c_data *obj = obj_i2c_data;
	int res = 0;
	...
	if(sensor_power == false)
	{
		res = KXTJ2_1009_SetPowerMode(kxtj2_1009_i2c_client, true);
		if(res)
		{
			GSE_ERR("Power on kxtj2_1009 error %d!\n", res);
		}
    ...
    for (i = 0; i < KXTJ2_1009_CALI_LEN; i++)
    {
		res = KXTJ2_1009_ReadData(kxtj2_1009_i2c_client, rawacc);
		if(res < 0)
		{        
			GSE_ERR("I2C error at %d: ret value=%d", i, res);
			return -3;
		}
		raw_sum[KXTJ2_1009_AXIS_X] += rawacc[KXTJ2_1009_AXIS_X];
		raw_sum[KXTJ2_1009_AXIS_Y] += rawacc[KXTJ2_1009_AXIS_Y];
		raw_sum[KXTJ2_1009_AXIS_Z] += rawacc[KXTJ2_1009_AXIS_Z];
		msleep(10);
    }
	// calc avarage
	GSE_LOG("sum,x:%d,y:%d,z:%d\n",raw_sum[KXTJ2_1009_AXIS_X],raw_sum[KXTJ2_1009_AXIS_Y],raw_sum[KXTJ2_1009_AXIS_Z]);
	rawacc[KXTJ2_1009_AXIS_X] = raw_sum[KXTJ2_1009_AXIS_X] / KXTJ2_1009_CALI_LEN;
	rawacc[KXTJ2_1009_AXIS_Y] = raw_sum[KXTJ2_1009_AXIS_Y] / KXTJ2_1009_CALI_LEN;
	rawacc[KXTJ2_1009_AXIS_Z] = raw_sum[KXTJ2_1009_AXIS_Z] / KXTJ2_1009_CALI_LEN;
	GSE_LOG("avg,x:%d,y:%d,z:%d\n",rawacc[KXTJ2_1009_AXIS_X],rawacc[KXTJ2_1009_AXIS_Y],rawacc[KXTJ2_1009_AXIS_Z]);
	
	//rawacc[KXTJ2_1009_AXIS_X] = (long)(rawacc[KXTJ2_1009_AXIS_X]) * obj->reso->sensitivity*GRAVITY_EARTH_1000/(1000*1000); //NTC
	//rawacc[KXTJ2_1009_AXIS_Y] = (long)(rawacc[KXTJ2_1009_AXIS_Y]) * obj->reso->sensitivity*GRAVITY_EARTH_1000/(1000*1000);
	//rawacc[KXTJ2_1009_AXIS_Z] = (long)(rawacc[KXTJ2_1009_AXIS_Z]) * obj->reso->sensitivity*GRAVITY_EARTH_1000/(1000*1000);

	cali_data[obj->cvt.map[KXTJ2_1009_AXIS_X]] = 0 - obj->cvt.sign[KXTJ2_1009_AXIS_X] * rawacc[KXTJ2_1009_AXIS_X];
	cali_data[obj->cvt.map[KXTJ2_1009_AXIS_Y]] = 0 - obj->cvt.sign[KXTJ2_1009_AXIS_Y] * rawacc[KXTJ2_1009_AXIS_Y];
	cali_data[obj->cvt.map[KXTJ2_1009_AXIS_Z]] = KXTJ2_1009_MAXZ - obj->cvt.sign[KXTJ2_1009_AXIS_Z] * rawacc[KXTJ2_1009_AXIS_Z];
    GSE_LOG("calidata,x:%d,y:%d,z:%d\n",cali_data[KXTJ2_1009_AXIS_X],cali_data[KXTJ2_1009_AXIS_Y],cali_data[KXTJ2_1009_AXIS_Z]);
	//check........
	if (0 == is_acc_calibrate_data_valid(cali_data))
	{
		GSE_ERR("calibration value invalid[X:%d,Y:%d,Z:%d].", cali_data[KXTJ2_1009_AXIS_X], 
			cali_data[KXTJ2_1009_AXIS_Y], cali_data[KXTJ2_1009_AXIS_Z]);
		return -2;
	}
	
	KXTJ2_1009_ResetCalibration(kxtj2_1009_i2c_client);
	KXTJ2_1009_WriteCalibration(kxtj2_1009_i2c_client, cali_data);
	
	cnt = sprintf(buffer, "%d %d %d", cali_data[KXTJ2_1009_AXIS_X], cali_data[KXTJ2_1009_AXIS_Y], cali_data[KXTJ2_1009_AXIS_Z]);
...
static ssize_t acc_calibrate_write_proc(struct file *file, const char __user *user_buf, size_t len, loff_t *offset)
{
	...
	sscanf(buf, "%d %d %d", &cali_data[KXTJ2_1009_AXIS_X], &cali_data[KXTJ2_1009_AXIS_Y], &cali_data[KXTJ2_1009_AXIS_Z]);
	GSE_LOG("[x:%d,y:%d,z:%d], copyLen=%d\n", cali_data[KXTJ2_1009_AXIS_X],
		cali_data[KXTJ2_1009_AXIS_Y],cali_data[KXTJ2_1009_AXIS_Z], (int)copyLen);
...
{
	const char *name = "mediatek,kxtj2-1009";
