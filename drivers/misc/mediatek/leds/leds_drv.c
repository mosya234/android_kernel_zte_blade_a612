...
#include <asm-generic/gpio.h> //ZTE_MODIFY
...
static ssize_t show_main_strobe_single_on(struct device *dev,struct device_attribute *attr,
					char *buf)
{
  return sprintf(buf, "%u\n", my_val);
}
static ssize_t store_main_strobe_single_on(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    char *pvalue = NULL;
     if(buf != NULL && size != 0)
    {
		...
		if(my_val==0)
			{
		main_strobe_single_off();
			}
		else{
		main_strobe_single_on();
	
			}
}
	 return size;
}
static DEVICE_ATTR(main_flashlight, 0664, show_main_strobe_single_on, store_main_strobe_single_on);
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		/*ZTE_MODIFY, add gpio control leds, start*/
		...
		/*ZTE_MODIFY, add gpio control leds, end*/
		g_leds_data[i] =
		...
		INIT_WORK(&g_leds_data[i]->work, mt_mt65xx_led_work);
              ...
              ret = device_create_file(&(pdev->dev), &dev_attr_main_flashlight);
		...
		if (strcmp(g_leds_data[i]->cdev.name, "lcd-backlight") == 0) {
			...
			bl_setting = &g_leds_data[i]->cust;
              ...
              ret = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_red_blink);
              }
              if (strcmp(g_leds_data[i]->cdev.name, "green") == 0) {
             ret = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_green_blink);
              }
              //zhangjian end add
		if (ret)
			goto err;
