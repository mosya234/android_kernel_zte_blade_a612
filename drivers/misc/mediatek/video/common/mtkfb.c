...
#include <linux/proc_fs.h> //yangchaofeng add
...
static int __parse_tag_videolfb(unsigned long node)
{
	...
	if (videolfb_tag) {
		...
		/*ZSW_MODIFY begin,modify for kernel eng ctrl,yangchaofeng,20160910*/
		#ifdef ZTE_KERNEL_ENG
		printk("%s:lcmname=%s\n",__FUNCTION__,videolfb_tag->lcmname);
		#endif
		/*ZSW_MODIFY end,modify for kernel eng ctrl,yangchaofeng,20160910*/
...
int _parse_tag_videolfb(void)
{
	...
	if (of_scan_flat_dt(fb_early_init_dt_get_chosen, &node) > 0) {
		...
		if (!ret) {
			...
			printk("[DT][videolfb] lcmfound=%d, fps=%d, fb_base=%p, vram=%d, lcmname=%s\n",
...
#define ILI9881C_SKYWIRTH_PANEL     "ili9881c_dsi_vdo_skyworth_hd720"
#define ILI9881C_YASHI_PANEL     "ili9881c_dsi_vdo_yashi_hd720"
#define ILI9881C_HOLITECH_PANEL     "ili9881c_dsi_vdo_holitech_hd720"
#define TD4100_BOE_PANEL     "td4100_dsi_vdo_boe_hd720"
...
static ssize_t lcd_info_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
   ...
   if (!strncmp(mtkfb_lcm_name, ILI9881C_DIJING_PANEL, strnlen(ILI9881C_DIJING_PANEL, PANEL_NAME_MAX_LEN)))
        strcpy(lcm_proc_info, "IC:ILI9881C; Vendor:dijing; Resolution:720*1280");
   else if(!strncmp(mtkfb_lcm_name, ILI9881C_SKYWIRTH_PANEL, strnlen(ILI9881C_SKYWIRTH_PANEL, PANEL_NAME_MAX_LEN)))
	strcpy(lcm_proc_info, "IC:ILI9881C; Vendor:skyworth; Resolution:720*1280");
   ...
   else if(!strncmp(mtkfb_lcm_name, ILI9881C_HOLITECH_PANEL, strnlen(ILI9881C_HOLITECH_PANEL, PANEL_NAME_MAX_LEN)))
	strcpy(lcm_proc_info, "IC:ILI9881C; Vendor:holiteck; Resolution:720*1280");
    else if(!strncmp(mtkfb_lcm_name, TD4100_BOE_PANEL, strnlen(TD4100_BOE_PANEL, PANEL_NAME_MAX_LEN)))
	strcpy(lcm_proc_info, "IC:ILI9881C; Vendor:boe; Resolution:720*1280");
   else
...
int  LCM_create_proc(void)
{
    ...
    if (!strncmp(mtkfb_lcm_name, ILI9881C_SKYWIRTH_PANEL, strnlen(ILI9881C_SKYWIRTH_PANEL, PANEL_NAME_MAX_LEN)))
        lcd_read_vendor_id = 0x15;
    ...
    if (mt_entry)
    {
        printk("create lcd_id success\n");
        return 1;
    }
    else
    {
        printk("create lcd_id fail\n");
        return 0;
    }
}
//add end
